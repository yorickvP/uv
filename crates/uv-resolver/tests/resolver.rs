...
use pep508_rs::{MarkerEnvironment, Requirement, StringVersion, UvRequirement};
...
async fn black_mypy_extensions() -> Result<()> {
    let manifest = Manifest::new(
        vec![UvRequirement::from_requirement(
            Requirement::from_str("black<=23.9.1").unwrap(),
        )],
...
async fn black_respect_preference() -> Result<()> {
    let manifest = Manifest::new(
        vec![UvRequirement::from_requirement(Requirement::from_str(
            "black<=23.9.1",
        )?)],
        Constraints::default(),
        Overrides::default(),
        vec![Preference::from_requirement(
            UvRequirement::from_requirement(Requirement::from_str("black==23.9.0")?),
        )],
        None,
        vec![],
        Exclusions::default(),
...
async fn black_ignore_preference() -> Result<()> {
    let manifest = Manifest::new(
        ...
        Overrides::default(),
        vec![Preference::from_requirement(
            UvRequirement::from_requirement(Requirement::from_str("black==23.9.2")?),
        )],
        None,
        vec![],
        Exclusions::default(),
...
async fn pylint_allow_explicit_prerelease_without_marker() -> Result<()> {
    let manifest = Manifest::simple(vec![
        UvRequirement::from_requirement(Requirement::from_str("pylint==2.3.0").unwrap()),
        UvRequirement::from_requirement(Requirement::from_str("isort>=5.0.0").unwrap()),
    ]);
    let options = OptionsBuilder::new()
...
async fn pylint_allow_explicit_prerelease_with_marker() -> Result<()> {
    let manifest = Manifest::simple(vec![
        UvRequirement::from_requirement(Requirement::from_str("pylint==2.3.0").unwrap()),
        UvRequirement::from_requirement(Requirement::from_str("isort>=5.0.0b").unwrap()),
    ]);
    let options = OptionsBuilder::new()
        .prerelease_mode(PreReleaseMode::Explicit)
        .exclude_newer(Some(*EXCLUDE_NEWER))
